...
lv_obj_t * Odometry::reset_button = lv_btn_create(Odometry::odom_container, NULL);
...
void Odometry::initialize(){
  ...
  lv_obj_set_pos(reset_button, 280, 20);
...
void Odometry::update(int l, int r, int b, double x, double y, double theta){
  double ticksPerInch = 900/(M_PI*4);
  double left = l/ticksPerInch;;
  double right = r/ticksPerInch;;

  lv_label_set_text(odom_l_enc_val, &std::to_string(left)[0]);
  lv_label_set_text(odom_r_enc_val, &std::to_string(right)[0]);
